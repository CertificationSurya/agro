...
const NewsCardGroup = ({apiUrl}: {apiUrl?: string}) => {
		...
		const fetchNews = async () => {
			try {
				const data = (await axios.get(
					`${apiUrl ? apiUrl : import.meta.env.VITE_API_URL}&apiKey=${
	...
	return (
		<div className={` container mx-auto px-4`}>
			...
			<div
				className={`grid sm:grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-4`}>
				{news.map((news, i) => (
